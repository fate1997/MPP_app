...
class OrganicLiquid:
    def __init__(self,  
                 task_unit={"Viscosity": "$\\rm{mPa·s}$", 
                            "Thermal conductivity": "$\\rm{W/(m·K)}$",  
                            "Diffusion coefficient in water": "$10^{-5} \\rm{cm^2/s}$"}):
    ...
    def show(self):
        image = Image.open('./image/organic_liquid.png')
    ...
    def compute(self, smiles, temperature, todo_tasks):
        ...
        for task in self.tasks:
            if todo_tasks[task]:
                if task == 'Diffusion coefficient in water':
                    task_value[task] = f'{torch.exp(self.models[task](data)).item() * 1e5: .3f}'           
                else:
